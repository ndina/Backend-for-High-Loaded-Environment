...
//we create an actor by calling content.actorOf()
//all actors have a common parent, the user guardian
//before you create an actor, Akka has already created three actors in the system -> root guardian, system guardian, user guardian
// root -> user -> system -> context
...
//postStop() is invoked just before the actor stops. No messages are processes after this point
...
object failureHanding extends App {
  val system = ActorSystem("testSystem")
